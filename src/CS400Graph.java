...
public class CS400Graph<NodeType, EdgeType extends Number> implements GraphADT<NodeType, EdgeType> {
    ...
    protected Path dijkstrasShortestPath(NodeType start, NodeType end) {
        if (start == null || end == null)
            throw new NoSuchElementException("Can not find path for null start or end");
        if (!containsVertex(start) || !containsVertex(end))
            throw new NoSuchElementException("Can not find path for start or end not exixt");
        PriorityQueue<Path> frontier = new PriorityQueue<Path>();
        LinkedList<Vertex> visited = new LinkedList<Vertex>();
        Path flight = new Path(vertices.get(start));
        frontier.add(flight);
        // check if visited vertex has edges
        while (!frontier.isEmpty()) {
            // visit shortest path
            flight = frontier.poll();
            if (flight.end.data.equals(end)) {
                return flight;
            }
            // Mark visited vertex
